defmodule KitchenRecipe.Accounts.User do
  ...
  def registration_changeset(user, attrs, opts \\ []) do
    user
    |> cast(attrs, [:email, :password, :username, :fullname, :role])
    |> validate_required([:username, :fullname, :role])
    |> validate_inclusion(:role, ~w(admin client))
    |> validate_length(:username, mix: 4)
    |> validate_length(:fullname, min: 4)
  ...
  defp maybe_validate_unique_email(changeset, opts) do
    if Keyword.get(opts, :validate_email, true) do
      changeset
      |> unsafe_validate_unique(:email, KitchenRecipe.Repo)
  ...
  """
  def valid_password?(%KitchenRecipe.Accounts.User{hashed_password: hashed_password}, password)
